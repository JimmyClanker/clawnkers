...
function buildFactRegistry(rawData = {}) {
  ...
  // Round 76 (AutoResearch): liquidity concentration risk from Round 73
  push('dex.liquidity_concentration_risk', rawData?.dex?.liquidity_concentration_risk);
  push('market.price_momentum_score', rawData?.market?.price_momentum_score);
  ...
  // Round 700 (AutoResearch batch): annualized fee/revenue yield relative to TVL
  // These help Grok understand capital efficiency and protocol productivity
  push('onchain.fee_to_tvl_annualized_pct', rawData?.onchain?.fee_to_tvl_annualized);
  push('onchain.revenue_to_tvl_annualized_pct', rawData?.onchain?.revenue_to_tvl_annualized);
  push('onchain.fee_capture_pct', rawData?.onchain?.fee_capture_pct);
  push('onchain.protocol_efficiency_score', rawData?.onchain?.protocol_efficiency_score);
  // FDV/MCap ratio as a derived signal (captures unlock risk better than raw numbers)
  const _fdv = rawData?.market?.fully_diluted_valuation ?? rawData?.market?.fdv;
  const _mcap = rawData?.market?.market_cap;
  if (_fdv > 0 && _mcap > 0) {
    push('market.fdv_to_mcap_ratio', parseFloat((_fdv / _mcap).toFixed(2)));
  }
  // Github velocity composite
  push('github.github_velocity_tier', rawData?.github?.github_velocity_tier);
  push('github.open_prs', rawData?.github?.open_prs);
  push('github.merged_prs_30d', rawData?.github?.merged_prs_30d);
...
export function buildDataSummary(rawData = {}) {
  ...
  if (!dex.error && rawData.dex) {
    ...
    // Round 99 (AutoResearch): liquidity concentration risk
    if (dex.liquidity_concentration_risk && dex.liquidity_concentration_risk !== 'low') {
      const concPct = dex.top_pair_liquidity_pct;
      const concDesc = dex.liquidity_concentration_risk === 'high' ? '🚨 High' : '⚠️ Elevated';
      const concDetail = concPct != null ? ` (${concPct.toFixed(0)}% of liquidity in single pair)` : '';
      dexLines.push(`- ${concDesc} liquidity concentration${concDetail} — single-pair rug risk`);
    }
...
function buildPrompt(projectName, rawData, scores) {
  ...
  return [
    ...
    '- bull_case: object with { thesis: string (2-3 sentences — the strongest argument FOR investing, with AT LEAST 3 specific numbers from RAW_DATA. Example: "With TVL of $XX growing Y% weekly, fee efficiency at $Z/M TVL, if protocol captures N% of the $ABC sector, mcap could reach $YY (P/TVL parity with sector leader)". If possible, derive an implied price target using concrete math.), price_target: OPTIONAL string (e.g., "$1.20-1.50 if TVL reaches $500M" — only if you can calculate it from comparable multiples. OMIT if data is sparse.), catalysts: array of 2-3 specific upcoming catalysts that could drive price up (be specific: "commit velocity 8/wk suggests Q2 mainnet release", not "increased adoption"), target_conditions: string (what needs to happen for the bull case to play out — be concrete: "TVL reaches $XX, fees sustain $YY/week, or sector rotation from BTC to DeFi"), probability: string ("high"/"medium"/"low" — how likely based on current data) }. Base this ONLY on verified data.',
    '- bear_case: object with { thesis: string (2-3 sentences — the strongest argument AGAINST investing, with AT LEAST 3 specific numbers from RAW_DATA. Example: "With top-10 holder concentration at X%, Y% unlock overhang pending, and DEX liquidity only $Z, a coordinated sell could drive -N% correction". If possible, estimate downside magnitude.), risks: array of 2-3 specific risks that could drive price down, failure_conditions: string (what would confirm the bear case — be specific: "Team unlock of X% supply, BTC dominance >60%, or protocol hack"), probability: string ("high"/"medium"/"low" — how likely based on current data) }. Be brutally honest.',
...
function normalizeReport(payload, projectName, rawData, scores) {
  ...
  return {
    ...
    bull_case: payload?.bull_case ? {
      ...
      // Round AutoResearch R1: capture price target if model provides it for quantitative orientation
      price_target: payload.bull_case.price_target ? String(payload.bull_case.price_target).trim() : null,
    } : null,
...
export function validateReport(report, rawData, scores = null) {
  ...
  if (report.moat && typeof report.moat === 'string') {
    const forbiddenMoatPhrases = [
      ...
      /\bwell.?positioned\b(?! (with \$|to capture \d))/i, // allow "well-positioned to capture 15%" but not bare "well-positioned"
      /\bexperienced team\b(?! with \d)/i, // allow "experienced team with 12 contributors" but not bare "experienced team"
      /\bgrowing (adoption|traction)\b(?! (\d|at \$))/i, // allow "growing adoption at 20% weekly" but not bare "growing adoption"
    ];
...
export function buildOpusPrompt(projectName, rawData, scores) {
  ...
  const system = [
    ...
    'Return ONLY valid JSON. Required fields:',
    '- verdict: "STRONG BUY" | "BUY" | "HOLD" | "AVOID" | "STRONG AVOID"',
    '- project_summary: 2-3 sentences MAX. Sentence 1: MUST name the protocol type, its primary blockchain/chain from ECOSYSTEM data (check ecosystem.primary_chain, ecosystem.chains, or dex.dex_chains for multi-chain protocols), and core use case (e.g. "X is a lending protocol on Ethereum and Base" or "Y is a DEX aggregator deployed on Arbitrum, Optimism, and Polygon"). If category unclear from RAW_DATA, use the most specific label you can infer from TVL location, GitHub repo name, or X_SOCIAL description. Sentence 2: the single most important traction metric from RAW_DATA — prioritize TVL > fees > MCap rank > GitHub commits (e.g. "$XM TVL growing +Y% weekly" or "ranked #Z by MCap with $XM 24h volume" or "generating $W fees/week with P/TVL of Q vs sector median R"). Sentence 3 (optional): key differentiator or primary user type, derived from moat analysis or sector positioning. Use only RAW_DATA and X_SOCIAL — no hype, no source tags, no vague claims.',
    ...
    '- analysis_text: 3-4 clean paragraphs for HUMAN READERS. STRICT PARAGRAPH SCOPES — each paragraph covers EXCLUSIVE territory, NO metric appears in 2 paragraphs:\n  Para 1 [VERDICT + TOP METRICS]: State verdict explicitly. Use the 2-3 highest-signal metrics (usually: overall score, P/TVL or market cap rank, and the single strongest signal — TVL growth OR social spike OR dev velocity). End Para 1 with a one-sentence verdict justification.\n  Para 2 [FUNDAMENTALS DEEP-DIVE]: On-chain numbers ONLY — TVL change, fees, revenue, protocol health, supply metrics. NEVER re-state Para 1 numbers. Add context: compare fees to peers if sector data available, highlight stickiness or growth trajectory.\n  Para 3 [MARKET + SENTIMENT]: Price action (24h/7d/30d), DEX pressure, social signal, volume patterns. NEVER re-state on-chain numbers from Para 2.\n  Para 4 [OUTLOOK]: Near-term risk/reward — what specific data threshold would UPGRADE or DOWNGRADE the verdict? Be concrete: "If TVL crosses $XM or fees sustain $Y/week, upgrade to BUY" OR "If price breaks $Z or holder concentration exceeds N%, downgrade to AVOID". State the primary catalyst to watch (e.g., "Q2 mainnet launch suggested by commit velocity") AND the main risk trigger (e.g., "team unlock of X% supply in 60d"). Use actual numbers from RAW_DATA.\n  FORMAT: $414.8M not 414828652 | -2.48% not -2.484401525322113% | NO source tags | NO snake_case field names.',
    '- moat: competitive advantage in 1-2 sentences. MANDATORY: use EXACTLY ONE evidence-backed moat type with specific numbers from RAW_DATA:\n  → SCALE MOAT (if SECTOR_CONTEXT available): "Leads category by TVL at $X, 2.5x larger than #2 competitor at $Y. Captures Z% of sector volume/fees." Use only when SECTOR_CONTEXT has comparative data — cite sector rank, relative TVL, or fee capture share.\n  → DEV/TECH MOAT (if github data available): "Active dev team: X contributors, Y commits/week, Z GitHub stars — typical category protocol averages W commits/week. Bus factor score A/100 (low key-person risk)." Must cite github.contributors, github.commit_frequency or commits_90d, optionally bus_factor_score for team resilience.\n  → LIQUIDITY/NETWORK MOAT (if DEX/onchain data available): "$XM deep liquidity across Z chains + Y% revenue capture (vs $W sector median) creates LP switching costs. Volume/liquidity ratio N% signals capital efficiency." Must use dex_liquidity_usd, chain_count, and optionally compare fee efficiency vs sector benchmark if SECTOR_CONTEXT exists.\n  → TOKENOMICS MOAT (if circulating supply data available): "Z% circulating supply with only Y% annual inflation gives controlled dilution vs category avg W%. X% revenue capture yields real yield > inflation." Must use pct_circulating, inflation_rate, and ideally revenue_to_fees_ratio to show sustainable tokenomics.\n  → DATA/PARTNERSHIP MOAT (if top_tier_source_count or narrative_dominance_score available): "Covered by N tier-1 media outlets and X institutional reports. Narrative dominance score Y/100 signals mainstream legitimacy vs competitors." Use top_tier_source_count from FACT_REGISTRY + narrative_dominance_score if available.\n  ABSOLUTE FORBIDDEN (delete on sight): "first mover advantage", "strong community" (without holder count or engagement score), "network effects" (without user/TVL numbers), "innovative", "unique", "robust ecosystem", "great team" (without github metrics).\n  If NO genuine moat exists from data: write EXACTLY "No clear moat identified from available data — competitive position unverifiable without sector comparison."',
    '- risks: array of 3-5 risk strings. Use format: "[SEVERITY] [Risk Type]: [specific detail WITH a number from RAW_DATA]. [concrete price impact if risk materializes WITH magnitude estimate]". Severity labels: [CRITICAL] / [HIGH] / [MEDIUM] / [LOW]. Examples: "[HIGH] Dilution: 42% circulating supply means 58% unlocking over 18 months = sustained sell pressure of ~3.2% monthly. Potential -25-35% price correction if team unlock accelerates." | "[CRITICAL] DEX Sell pressure: buy/sell ratio 0.78 over 24h suggests active distribution from large holders with $XM daily volume. If ratio falls below 0.6, expect -15-20% cascade." | "[HIGH] TVL Concentration: 73% TVL on single chain creates bridge/exploit single-point-of-failure. Historical bridge hacks average -40-60% price impact in 48h." ABSOLUTELY FORBIDDEN: "[MEDIUM] Regulatory uncertainty" (no context), "[LOW] Smart contract risk" (no audit data), any risk without a specific number. Risks without numbers = useless. Risks without severity = unranked noise. Quantify downside when sector precedent or historical pattern exists in SECTOR_CONTEXT or RAW_DATA.',
    '- catalysts: array of 2-4 upcoming catalysts. Format: "[Catalyst Type]: [specific observation from data] → [expected price/usage impact with magnitude]". Examples: "[Protocol upgrade]: Active dev team (Y commits/week) with recent release suggests protocol updates are in pipeline → potential +15-25% TVL inflow if adopted (based on prior upgrade patterns in SECTOR_CONTEXT)" | "[Revenue inflection]: Fee efficiency $X/M TVL vs sector average $Y — approaching breakeven threshold at $Z weekly fees that could trigger fee switch discussion → potential +30% price re-rating on revenue unlock" | "[Valuation re-rating]: P/TVL of Z is below sector median W — institutional discovery via tier-1 media coverage (X articles in 30d) could drive re-rating → potential 1.5-2x multiple expansion to sector parity". ONLY include catalysts supported by RAW_DATA patterns. Prefix uncertain ones with "[Unverified]". Quantify impact when comparable data exists. Having 1-2 strong data-backed catalysts with impact estimates > 4 generic ones.',
    '- competitor_comparison: Compare ONLY with metrics you have data for. If no competitor data available, write "Insufficient data for competitor comparison."',
    ...
    '- key_findings: array of 3-5 key findings. Format: "[Metric]: [value] — [what this means for investors]". Examples: "TVL: $45.2M growing +8% weekly — capital inflows suggest growing protocol adoption and positive sentiment" OR "P/TVL: 0.82x vs sector median 1.2x — protocol is undervalued relative to locked capital, potential re-rating catalyst" OR "Fee efficiency: $12/M TVL/week vs sector avg $8 — high capital productivity signals strong product-market fit" OR "FDV/MCap: 8.3x — 88% of supply still locked, massive dilution overhang on current holders". Each MUST cite a specific number from RAW_DATA or X_SOCIAL. PRIORITIZE comparative findings (vs sector benchmarks) when SECTOR_CONTEXT is available. Round 700 update: If fdv_to_mcap_ratio is available and > 3x, it MUST appear as a key finding with explicit dilution warning. If fee_to_tvl_annualized_pct > 10%, it MUST appear as a positive key finding.',
    '- liquidity_assessment: Based on RAW_DATA volume, market cap, and DEX liquidity. Do not invent slippage estimates.',
    ...
    '- Tokenized RWA (real-world assets): institutional capital inflow to on-chain equities, bonds, real estate — look for BlackRock/Franklin Templeton references',
    '- AI x Crypto convergence (on-chain AI inference, agent economy, ZKML, decentralized GPU compute): check if fee_to_tvl_annualized is high — compute demand is growing fast',
    '- Agent Commerce Protocol (ACP/x402): machine-to-machine payment flows — new primitive for autonomous spending',
    '- Stablecoin regulation (US GENIUS Act / STABLE Act): legitimacy boost for USD-pegged stablecoins; also benefits compliant DeFi protocols',
    '- Hyperliquid ecosystem dominance: perps DEX with on-chain orderbook — check if competing/referencing HLP vault',
    '- EVM account abstraction (ERC-4337/EIP-7702): better UX epoch — projects with paymaster support or bundler infrastructure gain',
    '- Restaking / LRT (liquid restaking tokens): ETH staker yield stack — check tvl_stickiness and protocol_efficiency_score for quality signal',
    '- Based rollups & preconfirmations: Ethereum L2 competitive shift — sequencer decentralization matters',
    '- Bitcoin DeFi (BTCfi): yield on native BTC without wrapping — check if protocol operates on Bitcoin L2s',
    '- Consumer crypto apps: retail-facing protocols with high active_users metrics and low protocol complexity',
    '- Cross-chain liquidity unification (Omni/LayerZero/Wormhole): check chain_count and dex_chains — omnichain presence is structural moat',
    'Only mention narrative alignment when the project data actually supports it. Do NOT force-fit a narrative that the data does not confirm.',
  ].join('\n\n');
...
export async function generateQuickReport(projectName, rawData, scores, { apiKey: explicitKey } = {}) {
  ...
  const prompt = [
    ...
    '- analysis_text: 2-3 clean paragraphs for HUMAN READERS. STRICT PARAGRAPH SCOPES — each paragraph covers EXCLUSIVE territory, NO metric appears in 2 paragraphs:\n  Para 1 [VERDICT + TOP SIGNAL]: State verdict. Cite the single highest-signal metric (P/TVL, TVL growth %, or fee efficiency). End with one-sentence verdict justification.\n  Para 2 [SUPPORTING DATA]: 2-3 different numbers (price momentum, social spike, dev activity, or holder concentration). NEVER re-state Para 1 metric.\n  Para 3 [OUTLOOK]: Near-term risk/reward — what data threshold would change verdict? What\'s the primary catalyst or risk trigger? Be concrete with numbers.\n  FORMAT: $414.8M not 414828652 | -2.48% not -2.484401525322113% | NO source tags | NO snake_case.',
